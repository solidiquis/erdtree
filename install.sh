...
usage() {
  this=$1
  cat <<EOF
$this: download rust binaries for solidiquis/erdtree
...
PROJECT_NAME="erdtree"
SHORTHAND_PROJECT_NAME="et"
OWNER=solidiquis
REPO="erdtree"
BINARY=erdtree
FORMAT=tar.gz
...
if [ "$OS" = "darwin" ]; then
  ...
  TARBALL="${SHORTHAND_PROJECT_NAME}-v${VERSION}-${ARCH}-${OS}."tar.gz""
fi
if [ "$OS" = "linux" ]; then
